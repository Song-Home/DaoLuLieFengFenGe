...
class Down(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.cbam = CBAM(out_channels)  # 使用out_channels作为CBAM的通道数

    def forward(self, x):
        x = self.mp_conv(x)
        x = self.cbam(x)  # 在经过MaxPool和DoubleConv处理后的特征图上应用CBAM
        return x
...
class CBAM(nn.Module):
    ...
    def forward(self, x):
        ...
        return out
# UP, OutConv, 和 UNet的定义保持不变
# 请确保在使用此修改后的UNet模型时，其他部分（如Up, OutConv等）仍然正确对接。
class Up(nn.Module):
